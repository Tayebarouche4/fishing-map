...
current-50 (أعماق، عمق ~50م): نفس الأسلوب تمامًا — 5 ملفات في
currents/current50/ بصيغة (current50_YYYY-MM-DD_HHh00Z.geojson).
...
def current50_filename(step_dt):
    """نفس منطق current1_filename لكن لتيار العمق (50م)."""
    return "current50_" + step_dt.strftime("%Y-%m-%d_%Hh00Z") + ".geojson"
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # ===== current-50 (أعماق): 5 محطات — نفس أسلوب current-1 تمامًا =====
        current50_folder = REPO_ROOT / "currents" / "current50"
        current50_folder.mkdir(parents=True, exist_ok=True)
        keep_names_50 = set()

        for offset in FORECAST_OFFSETS_HOURS:
            step_dt = now + timedelta(hours=offset)
            filename = current50_filename(step_dt)
            keep_names_50.add(filename)
            try:
                nc50_step = os.path.join(tmp, "current-50_+%02dh.nc" % offset)
                download_nc("current-50", step_dt, nc50_step)
                out50_step = current50_folder / filename
                count50 = process_layer(nc50_step, out50_step)
                print("current50 (+" + str(offset) + "h، " + step_dt.strftime("%H:00") + " UTC): " + str(count50) + " سهم")
            except Exception as e:
                print("تحذير: فشلت محطة +" + str(offset) + "h لتيار العمق، تُتخطى: " + str(e))
    ...
    cleanup_keep_only_current(current50_folder, keep_names_50)
    ...
    version_path.write_text(
        json.dumps(
            {"v": v, "msg": "تحديث تلقائي كل 3 ساعات — توقّع تيار السطح والعمق حتى +12 ساعة (5 محطات لكل منهما)"},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
