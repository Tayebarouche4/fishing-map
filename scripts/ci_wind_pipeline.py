...
def fetch_wind_blended():
    ...
    try:
        # محاولة واحدة فقط: فشل هذا النموذج غالباً بسبب عدم تغطية الموقع جغرافياً
        # (خارج نطاق AROME France)، وليس خطأ عابر يستفيد من إعادة المحاولة
        local = _fetch_model(MODEL_LOCAL_HD, MODEL_LOCAL_DAYS, retries=1)
    except Exception as e:
