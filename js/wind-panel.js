(function(){
  ...
  function fetchWind(showLoading){
      ...
      .catch(function(err){
        ...
        if(cache){ renderWind(cache.data); }
          ...
          body.innerHTML =
            ...
            + '<button class="wind-modal-refresh" onclick="window.__reloadWind()">إعادة المحاولة</button>'
            + '</div>';
        }
      });
  }
  window.__reloadWind = function(){ fetchWind(true); };
  ...
  function renderWind(payload){
    ...
    body.innerHTML = stripHtml
      + '<div class="wind-chart-scroll"><div class="wind-chart-box" id="wind-chart-box"><canvas id="wind-canvas"></canvas></div></div>'
  ...
  function drawWindChart(times, speeds, gusts, calmMax, moderateMax){
    ...
    // عرض حقيقي أكبر من الشاشة حسب عدد النقاط، مع لفافة تمرير أفقي (wind-chart-scroll)
    var PX_PER_HOUR = 10;
    var chartBox = document.getElementById('wind-chart-box');
    var minWidth = chartBox.parentElement.clientWidth || 300;
    chartBox.style.width = Math.max(minWidth, times.length * PX_PER_HOUR) + 'px';
    ...
    windChartInstance = new Chart(ctx, {
      ...
      options:{
        ...
        scales:{
          x:{ ticks:{ color:'#8a93ac', font:{family:'Tajawal',size:9},
                callback:function(v,i){ return i % 6 === 0 ? labels[i] : ''; }, maxRotation:0, autoSkip:false },
