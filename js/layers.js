...
// الأنواع المتوفر لها بيانات فعلياً — الكابلات مستثناة لعدم وجود ملف بيانات
var ALL_DANGER_TYPES = ['rocks', 'wrecks', 'debris'];

// الزر يظهر/يخفي كل الأخطار المتوفرة دفعة واحدة — بلا قائمة اختيار
function toggleDangersDropdown(tab) {
  var anyActive = activeDangers.size > 0;

  if (anyActive) {
    ALL_DANGER_TYPES.forEach(function(type) {
...
function loadDangerLayer(type) {
  ...
  if (!file) {
    ...
    setTimeout(function() {
      ...
      var cb = document.getElementById('danger-' + type);
