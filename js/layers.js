...
// في اتجاه التيار، فيختلف بين المحطتين حتى لو كانت نقطة الشبكة نفسها لم تتغيّر.
// النقطتان الأولى والرابعة من ضلع "الجسم" ثابتتان دائمًا حول مركز الخلية بغض
// النظر عن الاتجاه أو الطول — متوسطهما يعيد نقطة الشبكة الحقيقية بدقة.
function gridKeyFromFeature(feature) {
...
function buildCurrentTrendLayer(dataStart, dataEnd, startEntry, endEntry) {
  ...
  (dataEnd.features || []).forEach(function(f) {
    ...
    var speedStart = startByKey[g.key];
    if (speedStart == null) return;
    ...
    var marker = L.circleMarker([g.lat, g.lon], {
      radius: 5, color: color, weight: 1, fillColor: color, fillOpacity: 0.85
    });
    marker.bindPopup(
