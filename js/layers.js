...
// أرشيف تيار العمق (50م) — نفس أسلوب تيار السطح تمامًا: شريط تمرير زمني
// (الآن ← +12 ساعة، كل 3 ساعات)، بنفس صيغة اسم الملف وآلية القراءة
...
var current50TimeSteps    = null;  // [{name, utcDate, download_url}] مرتبة تصاعديًا حسب الوقت
var current50SliderOpen   = false;
var current50ActiveIndex  = null;  // index المحطة المعروضة حالياً على الخريطة
var current50Btn          = null;

function toggleCurrent50(btn) {
  current50Btn = btn;
  if (current50SliderOpen) { closeCurrent50Slider(); return; }
  openCurrent50Slider(btn);
}
...
function openCurrent50Slider(btn) {
  var panel = getCurrent50HistoryPanel();
  panel.style.display = 'block';
  current50SliderOpen = true;

  if (current50TimeSteps) {
    renderCurrent50Slider();
    return;
  }

  panel.innerHTML = '<div style="padding:10px;color:#94a3b8;">جاري التحميل...</div>';
  var listUrl = 'https://api.github.com/repos/' + CURRENTS_REPO_OWNER + '/' + CURRENTS_REPO_NAME
              + '/contents/' + CURRENT50_HISTORY_FOLDER;
  ...
  fetch(listUrl)
    ...
    .then(function(items) {
      ...
      if (steps.length === 0) {
        panel.innerHTML = '<div style="padding:10px;color:#94a3b8;">لا توجد بيانات تيار حالياً في مجلد "' + CURRENT50_HISTORY_FOLDER + '"</div>';
        return;
      }
      current50TimeSteps = steps;
      current50ActiveIndex = 0;
      renderCurrent50Slider();
    })
    .catch(function(e) {
      console.error('تعذر جلب أرشيف تيار العمق:', e);
      panel.innerHTML = '<div style="padding:10px;color:#f87171;">تعذر تحميل الأرشيف — تحقق من الاتصال أو من وجود المجلد "' + CURRENT50_HISTORY_FOLDER + '"</div>';
    });
}

function closeCurrent50Slider() {
  var panel = document.getElementById('current50-history-panel');
  if (panel) panel.style.display = 'none';
  current50SliderOpen = false;
}

function getCurrent50HistoryPanel() {
  var panel = document.getElementById('current50-history-panel');
  if (!panel) {
    panel = document.createElement('div');
    panel.id = 'current50-history-panel';
    panel.style.cssText = [
      'position:fixed', 'bottom:20px', 'left:210px', 'z-index:1060',
      'background:rgba(6,13,24,0.97)', 'border:1px solid rgba(139,92,246,0.35)',
      'border-radius:12px', 'padding:8px', 'font-family:Tajawal,sans-serif',
      'font-size:.75rem', 'color:#e2e8f0', 'direction:rtl', 'min-width:180px',
      'max-height:280px', 'overflow-y:auto', 'box-shadow:0 4px 16px rgba(0,0,0,0.5)'
    ].join(';');
    ...
    // إغلاق عند النقر خارج اللوحة — لا يُغلق عند النقر/السحب داخل الشريط نفسه
    document.addEventListener('click', function(ev) {
      if (!current50SliderOpen) return;
      var withinPanel = panel.contains(ev.target);
      var withinBtn   = current50Btn && current50Btn.contains(ev.target);
      if (!withinPanel && !withinBtn) closeCurrent50Slider();
    });
  }
  return panel;
}

function renderCurrent50Slider() {
  var panel = getCurrent50HistoryPanel();
  var steps = current50TimeSteps;
  var idx = (current50ActiveIndex != null) ? current50ActiveIndex : 0;

  var html = '<div style="font-weight:700;margin-bottom:8px;color:#fff;">🌊 تيار العمق — توقّع الساعات</div>';

  if (current50Visible) {
    html += '<div class="c50-hide" style="padding:6px 10px;margin-bottom:8px;border-radius:8px;'
      + 'cursor:pointer;background:rgba(255,255,255,0.08);color:#a78bfa;font-weight:700;text-align:center;">'
      + '&#10006; إخفاء الطبقة</div>';
  }

  html += '<div id="c50-slider-hour" style="text-align:center;font-weight:700;color:#fff;'
    + 'margin-bottom:6px;font-size:.95rem;">' + formatLocalHour(steps[idx].utcDate) + '</div>';

  html += '<input type="range" class="c50-slider" min="0" max="' + (steps.length - 1)
    + '" step="1" value="' + idx + '" style="width:100%;">';
  ...
  var hideBtn = panel.querySelector('.c50-hide');
  if (hideBtn) hideBtn.onclick = function() { hideCurrent50Layer(); };

  var slider = panel.querySelector('.c50-slider');
  slider.oninput = function() {
    var i = parseInt(this.value, 10);
    current50ActiveIndex = i;
    document.getElementById('c50-slider-hour').textContent = formatLocalHour(steps[i].utcDate);
    loadCurrent50Snapshot(steps[i]);
  };
}

function loadCurrent50Snapshot(entry) {
  ...
  if (current50LayerCache[entry.name]) {
    ...
    finishCurrent50Load(entry, hourLabel);
    return;
  }
...
function finishCurrent50Load(entry, hourLabel) {
  current50Visible = true;
  if (current50Btn) {
    current50Btn.style.background  = 'rgba(139,92,246,0.25)';
    current50Btn.style.borderColor = '#8b5cf6';
  }
}

function hideCurrent50Layer() {
  if (current50Layer) map.removeLayer(current50Layer);
  current50Visible = false;
  if (current50Btn) {
    current50Btn.style.background  = 'rgba(139,92,246,0.1)';
    current50Btn.style.borderColor = 'rgba(139,92,246,0.3)';
  }
  closeCurrent50Slider();
}
