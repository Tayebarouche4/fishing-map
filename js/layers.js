    ...
    .then(function(data) {
      current1Layer = L.geoJSON(data, {
        style: function(feature) {
          ...
          var color, fillColor;
          // أخضر زمردي = ضعيف ← أصفر = متوسط ← برتقالي = قوي ← أحمر = شديد
          if      (speed >= 0.50) { color = '#b91c1c'; fillColor = '#ef4444'; }  // أحمر — شديد
          else if (speed >= 0.35) { color = '#c2410c'; fillColor = '#fb923c'; }  // برتقالي — قوي
          else if (speed >= 0.15) { color = '#a16207'; fillColor = '#facc15'; }  // أصفر — متوسط
          else                    { color = '#047857'; fillColor = '#34d399'; }  // أخضر — ضعيف
          return { color, weight:3, opacity:0.9, fillColor, fillOpacity:0.5 };
        },
        onEachFeature: function(feature, layer) {
...
function showCurrent1Legend() {
  var existing = document.getElementById('current1-legend');
  ...
  el.innerHTML = [
    '<div style="font-weight:700;margin-bottom:8px;font-size:.8rem;">🌊 تيار السطح (م/ث)</div>',
    _legendRow('#34d399','ضعيف   < 0.15'),
    _legendRow('#facc15','متوسط  0.15 – 0.35'),
    _legendRow('#fb923c','قوي    0.35 – 0.50'),
    _legendRow('#ef4444','شديد   > 0.50')
  ].join('');
