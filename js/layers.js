    ...
    .then(function(data) {
      current1Layer = L.geoJSON(data, {
        style: function(feature) {
          ...
          var color, fillColor;
          if      (speed >= 0.50) { color = '#ffffff'; fillColor = '#dc2626'; }  // أحمر — شديد
          else if (speed >= 0.35) { color = '#ffffff'; fillColor = '#f97316'; }  // برتقالي — قوي
          else if (speed >= 0.15) { color = '#ffffff'; fillColor = '#facc15'; }  // أصفر — متوسط
          else                    { color = '#ffffff'; fillColor = '#ffffff'; }  // أبيض — ضعيف
...
function showCurrent1Legend() {
  ...
  el.innerHTML = [
    ...
    _legendRow('#ffffff', 'ضعيف   < 0.15'),
    _legendRow('#facc15', 'متوسط  0.15 – 0.35'),
    _legendRow('#f97316', 'قوي    0.35 – 0.50'),
    _legendRow('#dc2626', 'شديد   > 0.50')
  ].join('');
...
function _legendRow(color, label) {
  return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">'
    ...
    + 'background:' + color + ';flex-shrink:0;border:1px solid rgba(255,255,255,0.2);"></span>'
    + '<span>' + label + '</span></div>';
}
