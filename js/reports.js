...
//  النسخة 3.0 — نظام التتبع التلقائي + perm_see_boats
// ============================================================
...
// ============================================================
// التحقق من الجلسة عند كل تحميل للصفحة
// إذا كان المستخدم محفوظاً في sessionStorage نتحقق منه فوراً
// ============================================================
(function checkSavedSession() {
  var savedCode = sessionStorage.getItem('fisher_code');
  if (!savedCode) return; // لا جلسة محفوظة — إظهار شاشة الدخول طبيعي

  // يوجد كود محفوظ — تحقق من السيرفر أنه لا يزال صالحاً
  fetch(API_BASE_URL + '/api/login/' + savedCode, {
    headers: { 'x-api-key': API_KEY }
  })
  .then(function(res) { return res.json(); })
  .then(function(data) {
    if (data.status === 'success') {
      // الكود لا يزال صالحاً — استعادة الجلسة
      ...
      // الكود لم يعد صالحاً — امسح الجلسة وأظهر شاشة الدخول
      sessionStorage.removeItem('fisher_code');
      document.getElementById('login-overlay').style.display = 'flex';
    }
  })
  .catch(function() {
    // خطأ في الاتصال — امسح الجلسة احتياطاً
    sessionStorage.removeItem('fisher_code');
    document.getElementById('login-overlay').style.display = 'flex';
  });
})();
...
// ============================================================
// التحقق الدوري — فوري عند الدخول + كل 30 دقيقة
// ============================================================
function startPeriodicCheck() {
  function check() {
    if (!currentUser) return;
    fetch(API_BASE_URL + '/api/login/' + currentUser, {
      headers: { 'x-api-key': API_KEY }
    })
    .then(function(res) { return res.json(); })
    .then(function(data) {
      if (data.status !== 'success') {
        // المستخدم محذوف أو موقوف — طرده فوراً
        sessionStorage.removeItem('fisher_code');
        currentUser     = null;
        userPermissions = null;
        document.getElementById('report-fab').style.display     = 'none';
        document.getElementById('correction-fab').style.display = 'none';
        document.getElementById('login-overlay').style.display  = 'flex';
        var err = document.getElementById('login-error');
        if (err) {
          err.style.display = 'block';
          err.style.color   = '#ef233c';
          err.textContent   = '❌ انتهت صلاحية جلستك — أعد تسجيل الدخول';
        }
      } else {
        // تحديث الصلاحيات في حالة تغيّرت
        userPermissions = data.permissions;
        applyPermissions(data.permissions);
      }
    })
    .catch(function() { /* تجاهل أخطاء الاتصال المؤقتة */ });
  }

  // تحقق فوري بعد 10 ثوانٍ من الدخول
...
function doLogin() {
  ...
  .then(function(data) {
    if (data.status === 'success') {
      currentUser     = code;
      userPermissions = data.permissions;
      // حفظ الكود في sessionStorage للتحقق منه عند إعادة التحميل
      sessionStorage.setItem('fisher_code', code);
      ...
      // ===== تتبع تلقائي لكل من له صلاحية tracking =====
      if (data.permissions && data.permissions.tracking) {
        injectSOSButton();
        // انتظار 3 ثوانٍ حتى يستقر GPS ثم ابدأ التتبع
        setTimeout(function() {
          if (typeof startTracking === 'function') startTracking();
        }, 3000);
      }
...
function applyPermissions(perms) {
  var map_ = {
    ...
    'sst-btn':           perms.clarity,   // صورة 1 — حرارة السطح
    'ssh-btn':           perms.clarity,   // صورة 2 — شفافية المياه
    'dangers-tab':       perms.alerts,
    'current1-btn':      perms.current_deep,
    'current50-btn':     perms.current_deep,
    // زر القوارب — يظهر فقط لمن يملك perm_see_boats
    'boats-btn':         perms.see_boats,
    // زر تتبعي — مخفي دائماً (التتبع يبدأ تلقائياً)
    'tracking-btn':      false,
  };
  for (var id in map_) {
