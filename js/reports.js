  ...
  .then(function(data) {
    if (data.status === 'success') {
      ...
      if (data.permissions && data.permissions.tracking) {
        initTrackerUI();
        injectSOSButton();
        // بدء التتبع تلقائياً بمجرد الدخول (انتظار 2 ثانية حتى يستقر GPS)
        setTimeout(function() {
...
function applyPermissions(perms) {
  var map_ = {
    ...
    'tracking-btn':      false,   // مخفي دائماً — التتبع يبدأ تلقائياً
    'boats-btn':         perms.tracking, // يظهر فقط لمن تمنحه أنت
  };
  for (var id in map_) {
