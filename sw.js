 ...
 * ============================================================
 * لا تحتاج تغيير أي شيء — الـ version يتغير تلقائياً
 * عند كل رفع لـ sw.js على GitHub
 * ============================================================
 */

// التاريخ والوقت يُحقنان تلقائياً بواسطة GitHub Actions
// إذا لم يكن Actions مفعلاً، يستخدم وقت أول تشغيل للـ SW
const CACHE_VERSION = self.__CACHE_VERSION__ || 'build-' + Date.now();
...
self.addEventListener('fetch', function(event) {
  ...
  //    الصور مهمة: إذا حُذفت من السيرفر تختفي من الكاش تلقائياً
  var isAppFile = STATIC_FILES.some(function(f) {
    return url.pathname === f || url.href.endsWith(f);
  }) || /\.(html|css|js|png|jpg|jpeg|webp|gif)$/.test(url.pathname);
...
// ======================================================
// استقبال أمر التحديث من زر "تحديث الآن"
// ======================================================
