...
// ============================================================
// Network First — للبيانات اليومية (دائماً من الشبكة، لا cache)
// ============================================================
async function networkFirst(request) {
    try {
        // دائماً من الشبكة بدون cache — يضمن التحديث الفوري
        var response = await fetchWithTimeout(request.clone(), 10000);
        if (response.ok) {
            console.log('[SW] محدّث من الشبكة:', request.url.split('/').pop());
        }
        return response;
    } catch (err) {
        // فقط عند انقطاع الاتصال — ارجع للكاش
